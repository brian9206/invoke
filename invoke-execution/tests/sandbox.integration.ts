...
async function main(): Promise<void> {
 ...
 try {
  ...
  await test('IPC binary: ping-binary → pong-binary (buffer echo)', async () => {
    ...
    const [returnedPayload, returnedBuffer] = await new Promise<[unknown, Buffer]>(
      (resolve, reject) => {
        const timer = setTimeout(
          () => reject(new Error('Timeout (15s) waiting for pong-binary')),
          15_000
        );
  ...
  if (SKIP_NETWORK) {
    ...
    console.log(
      SANDBOX_RUNTIME === 'runc'
        ? '  note: using runc — NetworkMode=none means no network; expecting failures\n'
        : ''
    );
    ...
    await test('Network: allowed after filter update (filter=true)', async () => {
      try {
        await orchestrator.setNetwork({
          name: 'sb-net-test',
          rules: [
            { cidr: '192.168.0.0/16', action: 'RETURN' },
          ]
        });
