...
crypto.randomUUID = function(options) {
...
crypto.randomInt = function(min, max, callback) {
...
crypto.generateKeyPair = function(type, options, callback) {
...
crypto.sign = function(algorithm, data, privateKey) {
    const result = _crypto_sign.applySync(undefined, [algorithm, data, privateKey], { arguments: { copy: true } });
    return Buffer.from(result);
};

crypto.verify = function(algorithm, data, publicKey, signature) {
...
crypto.publicEncrypt = function(key, buffer) {
...
crypto.privateEncrypt = function(key, buffer) {
