  ...
  private async runBuild(build: any): Promise<void> {
    ...
    try {
      ...
      const onBuildContext = (payload: any) => {
        if (!payload) return
        FunctionBuild.update({ build_context: payload }, { where: { id: buildId } }).catch((err: any) => {
      ...
      try {
        ...
        const buildCompletePromise = new Promise<{
          success: boolean
          error?: string
          artifactHash?: string
          uploadResult?: any
      ...
      if (build.after_build_action === 'switch') {
        ...
        await database.sequelize.query(`SELECT pg_notify('execution_cache_invalidated', :payload::text)`, {
      ...
      await (FunctionVersion as any)
        .update({ build_status: 'failed' }, { where: { id: build.version_id } })
        .catch(() => {})
    } finally {
      // Clean up temp dir
