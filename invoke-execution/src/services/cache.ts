...
class CacheService {
  ...
  async checkCache(functionId: string, expectedHash: string, version: string): Promise<CacheCheckResult> {
    await this.initialize()
  ...
  async cachePackage(functionId: string, version: string, hash: string, size: number): Promise<string> {
    await this.initialize()
  ...
  async removeFromCache(functionId: string): Promise<void> {
    ...
    try {
      await Promise.all([fs.remove(cachedPath), fs.remove(extractedPath), fs.remove(metadataPath)])
  ...
  async evictLeastRecentlyUsed(): Promise<CleanupResult> {
    ...
    try {
      ...
      for (const pkg of packages) {
        ...
        removed++
        freedBytes += pkg.size
        ;(currentStats as any).totalSizeBytes = (currentStats.totalSizeBytes ?? 0) - pkg.size
