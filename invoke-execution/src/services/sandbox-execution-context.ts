...
export async function executeSandbox(
  ...
  const { functionId, request, env, timeoutMs, kvStore, projectSlug, consoleLogger } = options
  ...
  return new Promise<SandboxExecutionResult>((resolve, reject) => {
    ...
    const onExecuteResult = (payload: { response: ResponseData }) => {
      ...
      const bodyBuf = response.body ? Buffer.from(response.body, 'base64') : undefined
