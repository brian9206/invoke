...
class ExecutionServer {
  ...
  async start(): Promise<void> {
    try {
      ...
      validateEnvironment(['DB_NAME', 'DB_USER', 'DB_PASSWORD', 'S3_ENDPOINT', 'S3_ACCESS_KEY', 'S3_SECRET_KEY'])

      if (!process.env.INTERNAL_SERVICE_SECRET) {
        console.warn('⚠️  WARNING: INTERNAL_SERVICE_SECRET is not set. Gateway token verification is disabled.')
      }
