...
export default function GlobalSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    if (!Number.isInteger(defTimeout) || defTimeout < 10) {
      toast.error('Default timeout must be an integer ≥ 10 seconds.')
      return
    }
    if (!Number.isInteger(maxTimeout) || maxTimeout < 10) {
      toast.error('Max timeout must be an integer ≥ 10 seconds.')
      return
    }
    if (maxTimeout < defTimeout) {
      toast.error('Max timeout must be ≥ default timeout.')
      return
    }
    const maxBuilds = Number(maxConcurrentBuilds)
    if (!Number.isInteger(maxBuilds) || maxBuilds < 1) {
      toast.error('Max concurrent builds must be an integer ≥ 1.')
      return
    }
    ...
    if (!isValidBuildMemory) {
      toast.error('Build memory must be a multiple of 256 MB, minimum 256 MB.')
      return
    }
    const isAligned = (n: number) => Number.isInteger(n) && n >= 256 && n % 256 === 0
    if (isNaN(defMemory) || !isAligned(defMemory)) {
      toast.error('Default memory must be a multiple of 256 MB and at least 256 MB.')
      return
    }
    if (isNaN(maxMemory) || !isAligned(maxMemory)) {
      toast.error('Max memory must be a multiple of 256 MB and at least 256 MB.')
      return
    }
    if (maxMemory < defMemory) {
      toast.error('Max memory must be ≥ default memory.')
      return
    }
                  ...
                  <div className='space-y-4'>
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Limit the total key-value store size per project.
                      </p>
                    </div>
                ...
                <CardContent className='pt-6 space-y-8'>
                  ...
                  <div className='space-y-4'>
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Control how long functions are allowed to execute.
                      </p>
                    </div>
                    <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                      <div className='space-y-1.5'>
                        ...
                        <p className='text-xs text-muted-foreground'>
                          Used when a function has no custom timeout. Minimum: 10s.
                        </p>
                      </div>
                      <div className='space-y-1.5'>
                        ...
                        />
                        <p className='text-xs text-muted-foreground'>
                          Upper bound for per-function custom timeouts. Must be ≥ default.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='space-y-4'>
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Control the isolate memory budget per function. Values must be multiples of 256 MB (256, 512,
                        768, 1024, …).
                      </p>
                    </div>
                    <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                      <div className='space-y-1.5'>
                        ...
                        />
                        <p className='text-xs text-muted-foreground'>
                          Used when a function has no custom memory limit. Minimum: 256 MB.
                        </p>
                      </div>
                      <div className='space-y-1.5'>
                        <Label>Maximum Memory Limit</Label>
                        <MemoryInput value={execMaxMemory} onChange={setExecMaxMemory} placeholder='e.g. 1G or 2G' />
                        <p className='text-xs text-muted-foreground'>
                          Upper bound for per-function custom memory limits. Must be ≥ default.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <CardContent className='pt-6 space-y-8'>
                  ...
                  <div className='space-y-4'>
                    ...
                    <div className='max-w-xs space-y-1.5'>
                      ...
                      />
                      <p className='text-xs text-muted-foreground'>
                        Maximum number of builds that can run simultaneously. Minimum: 1.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className='space-y-4'>
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Cgroup memory limit applied to each build sandbox. Must be a multiple of 256 MB.
                      </p>
                    </div>
                    <div className='max-w-xs space-y-1.5'>
                      ...
                      />
                      <p className='text-xs text-muted-foreground'>
                        Minimum 256 MB, multiples of 256 MB (e.g. 512, 1024, 2048).
                      </p>
                    </div>
                  </div>
                ...
                <CardContent className='pt-6 space-y-8'>
                  <div className='space-y-4'>
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Configure automatic cleanup of execution logs.
                      </p>
                    </div>
                    <div className='flex items-center gap-3'>
                      <Switch id='retentionEnabled' checked={retentionEnabled} onCheckedChange={setRetentionEnabled} />
                      <Label htmlFor='retentionEnabled' className='cursor-pointer'>
                    ...
                    {retentionEnabled && (
                      <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 pl-2'>
                        <div className='space-y-1.5'>
                          ...
                          <Select
                            value={retentionType}
                            onValueChange={v => setRetentionType(v as 'time' | 'count' | 'none')}
                          >
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                  ...
                  <div className='flex flex-wrap items-center gap-3 pt-1'>
                    ...
                    <Button variant='destructive' onClick={handleCleanup} disabled={cleaning || !retentionEnabled}>
                      {cleaning ? (
                        <>
                ...
                <CardContent className='pt-6 space-y-8'>
                  <div className='space-y-4'>
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Configure the API Gateway for routing HTTP traffic to functions.
                      </p>
                    </div>
                    <div className='flex items-center gap-3'>
                      <Switch
                        id='apiGatewayEnabled'
                        checked={apiGatewayEnabled}
                        onCheckedChange={setApiGatewayEnabled}
                      />
                      <Label htmlFor='apiGatewayEnabled' className='cursor-pointer'>
                        Enable API Gateway
                      </Label>
                    </div>
                    {apiGatewayEnabled && (
                      <div className='space-y-1.5'>
                        ...
                        <div className='flex gap-2'>
                          <Select
                            ...
                            onValueChange={v => setApiGatewayDomainProtocol(v as 'http' | 'https')}
                          >
                  ...
                  <div className='flex flex-wrap items-center gap-3 pt-1'>
                    <Button onClick={handleSave} disabled={saving || (apiGatewayEnabled && !apiGatewayDomain.trim())}>
                      {saving ? (
                        <>
                          <Loader className='w-4 h-4 mr-2 animate-spin' /> Saving…
                        </>
