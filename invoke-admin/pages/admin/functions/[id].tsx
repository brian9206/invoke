...
export default function FunctionDetails() {
  ...
  const [scheduleSettings, setScheduleSettings] = useState({
    ...
    schedule_cron: '',
    next_execution: null as any,
    last_scheduled_execution: null as any
  })
  ...
  const [retentionSettings, setRetentionSettings] = useState({
    retention_type: null as any,
    retention_value: null as any,
  ...
  const [globalExecSettings, setGlobalExecSettings] = useState({
    defaultTimeout: 30,
    maxTimeout: 60,
    defaultMemory: 256,
    maxMemory: 1024
  ...
  const advancedHasChanges = () => {
    ...
    return (
      ...
      customTimeoutSeconds !==
        (functionData.custom_timeout_seconds != null ? String(functionData.custom_timeout_seconds) : '') ||
  ...
  const handleTabChange = (next: string) => {
    const dirty =
      (activeTab === 'schedule' && scheduleHasChanges()) || (activeTab === 'advanced' && advancedHasChanges())
  ...
  useEffect(() => {
    if (functionData?.id) getFunctionUrl(functionData.id).then(setFunctionUrl)
  }, [functionData?.id])
  ...
  const fetchRetentionSettings = async () => {
    try {
      ...
      console.error('Error fetching retention settings')
    }
  }
  ...
  const fetchEnvironmentVariables = async () => {
    ...
    try {
      ...
      const d = await r.json()
      if (d.success) {
        setEnvironmentVariables(d.data)
        setTempEnvVars(d.data)
      }
    } catch {
      console.error('Error fetching env vars')
    } finally {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setDialogState({
          type: 'alert',
          title: 'Error',
          message: 'Failed to save: ' + (d.error || d.message || 'Unknown')
        })
      }
    } catch {
      setDialogState({ type: 'alert', title: 'Error', message: 'Error saving function' })
    } finally {
  ...
  const toggleActiveStatus = async () => {
    try {
      const r = await authenticatedFetch(`/api/functions/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ is_active: !functionData?.is_active })
      })
  ...
  const handleSwitchVersion = async (versionId: string, version: string) => {
    setDialogState({
      ...
      onConfirm: async () => {
        ...
        try {
          const r = await authenticatedFetch(`/api/functions/${id}/switch-version`, {
            method: 'POST',
          ...
          if (r.status === 202 && result.buildRequired) {
            setDialogState({
              type: 'alert',
              title: 'Build Queued',
              message: `Version ${version} has not been built yet. A build has been queued and will automatically activate this version when complete.`
            })
            fetchVersions()
          ...
          setDialogState({ type: 'alert', title: 'Error', message: 'Network error occurred' })
        } finally {
  ...
  const handleDeleteVersion = async (versionId: string, versionNumber: number) => {
    setDialogState({
      ...
      onConfirm: async () => {
        ...
        try {
          const r = await authenticatedFetch(`/api/functions/${id}/versions?version=${versionNumber}`, {
            method: 'DELETE'
          })
          const result = await r.json()
          if (result.success) {
            ...
            setDialogState({ type: 'alert', title: 'Error', message: result.message || 'Failed to delete version' })
        } catch {
          setDialogState({ type: 'alert', title: 'Error', message: 'Network error occurred' })
        } finally {
  ...
  const handleCleanupVersions = () => {
    ...
    if (toDelete.length === 0) {
      setDialogState({
        ...
        message: 'Everything is clean. Nothing needs to be deleted.'
      })
      return
    }
    setDialogState({
    ...
    try {
      ...
      if (d.success) {
        ...
        setDialogState({
          type: 'alert',
          title: 'Error',
          message: 'Failed to save environment variables: ' + (d.error || d.message || 'Unknown')
        })
    } catch {
    ...
    try {
      ...
      if (d.success) {
        ...
        setDialogState({
          type: 'alert',
          title: 'Error',
          message: 'Failed to save schedule: ' + (d.error || d.message || 'Unknown')
        })
      }
    } catch {
      setDialogState({ type: 'alert', title: 'Error', message: 'Error saving schedule' })
    } finally {
  ...
  const saveRetentionSettings = async () => {
    ...
    try {
      ...
      const d = await r.json()
      if (d.success) {
        setRetentionSettings(retentionDraft as any)
        setRetentionModalOpen(false)
      } else
        setDialogState({
          type: 'alert',
          title: 'Error',
          message: 'Failed to save retention: ' + (d.error || d.message || 'Unknown')
        })
    } catch {
      setDialogState({ type: 'alert', title: 'Error', message: 'Error saving retention settings' })
    } finally {
  ...
  const regenerateApiKey = async () => {
    ...
    try {
      ...
      console.error('Error regenerating API key')
    } finally {
  ...
  const saveExecutionLimits = async () => {
    ...
    if (customTimeoutEnabled) {
      if (!Number.isInteger(timeoutVal) || (timeoutVal as number) < 10) {
        const { toast } = await import('sonner')
        toast.error('Timeout must be an integer ≥ 10 seconds.')
        return
      }
      if ((timeoutVal as number) > globalExecSettings.maxTimeout) {
        const { toast } = await import('sonner')
        toast.error(`Timeout must be ≤ global max (${globalExecSettings.maxTimeout}s).`)
        return
      }
    }
    ...
    if (customMemoryEnabled) {
      if (memoryVal === null || isNaN(memoryVal as number)) {
        const { toast } = await import('sonner')
        toast.error('Enter a valid memory value (e.g. 512 or 512M or 1G).')
        return
      }
      if ((memoryVal as number) < 256 || (memoryVal as number) % 256 !== 0) {
        const { toast } = await import('sonner')
        toast.error('Memory must be a multiple of 256 MB and at least 256 MB.')
        return
      }
      if ((memoryVal as number) > globalExecSettings.maxMemory) {
        const { toast } = await import('sonner')
        toast.error(`Memory must be ≤ global max (${globalExecSettings.maxMemory} MB).`)
        return
      }
    }
    ...
    try {
      ...
      if (r.ok) {
        ...
        const d = await r.json()
        const { toast } = await import('sonner')
        toast.error(d.error || d.message || 'Failed to save execution limits.')
      }
    } catch {
      const { toast } = await import('sonner')
      toast.error('Error saving execution limits.')
    } finally {
      setExecLimitsSaving(false)
    }
  }
  ...
  const getStatusBadgeVariant = (code: number) =>
    code >= 200 && code < 300 ? 'success' : code >= 400 ? 'destructive' : 'warning'
  const getGatewayUrls = (): string[] => {
    ...
    const gatewayFull = gatewayDomain
      ? gatewayDomain.startsWith('http')
        ? gatewayDomain.replace(/\/+$/, '')
        : `https://${gatewayDomain.replace(/\/+$/, '')}`
      : ''
    const customFull = gatewayCustomDomain
      ? gatewayCustomDomain.startsWith('http')
        ? gatewayCustomDomain.replace(/\/+$/, '')
        : `https://${gatewayCustomDomain.replace(/\/+$/, '')}`
      : ''
    const urls: string[] = []
  ...
  if (!functionData)
    return (
      <ProtectedRoute>
        <Layout>
          <div className='flex items-center justify-center h-64 text-destructive'>Function not found</div>
        </Layout>
      </ProtectedRoute>
    )
          ...
          <div>
            ...
            {functionData.project_is_active === false && (
              <div className='flex items-center gap-3 rounded-lg border border-yellow-600/50 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-400'>
                ...
                <span>
                  The project <strong>{functionData.project_name}</strong> is currently inactive. This function cannot
                  be executed until the project is reactivated.
                </span>
              </div>
            )}
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        size='icon'
                        onClick={() => {
                          setEditName(functionData.name)
                          setEditDescription(functionData.description || '')
                  ...
                  <CardContent className='space-y-2'>
                    <div className='flex min-w-0'>
                      <Input
                        ...
                        value={functionUrl}
                        readOnly
                        className='min-w-0 flex-1 rounded-r-none font-mono text-xs'
                      />
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                  ...
                  <CardContent>
                    <dl className='grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm'>
                      <div className='flex flex-col gap-0.5'>
                        ...
                        <dd>
                          <Badge variant={functionData.is_active ? 'success' : 'secondary'}>
                            {functionData.is_active ? 'Active' : 'Inactive'}
                          </Badge>
                        </dd>
                      </div>
                      <div className='flex flex-col gap-0.5'>
                        ...
                        <dd className='font-mono'>
                          {functionData.active_version != null ? `v${functionData.active_version}` : '—'}
                        </dd>
                      </div>
                      <div className='flex flex-col gap-0.5'>
                        <dt className='text-xs font-medium text-muted-foreground uppercase tracking-wide'>
                                  ...
                                  <TableCell>
                                    <DropdownMenu>
                                      ...
                                      <DropdownMenuContent align='end'>
                                        <DropdownMenuItem
                                          onClick={() =>
                                            router.push(`/admin/functions/${id}/versions/${version.id}/edit`)
                                          }
                                        >
                                        ...
                                        {!version.is_active && (
                                          <DropdownMenuItem
                                            ...
                                            disabled={
                                              ...
                                              version.build_status === 'running' ||
                                              version.build_status === 'queued'
                                            }
                                          >
                      ...
                      <div className='space-y-4'>
                        ...
                        <div className='bg-blue-900/20 border border-blue-700 rounded-lg p-3'>
                          <p className='text-blue-300 text-sm'>
                            <strong>Note:</strong> Variables are available as{' '}
                            <code className='text-blue-200 bg-blue-900/50 px-1 rounded'>process.env.VARIABLE_NAME</code>
                          </p>
                        </div>
                      </div>
                        ...
                        <div className='flex items-center justify-between gap-4'>
                          <div className='flex-1'>
                            ...
                            <p className='text-sm text-muted-foreground mt-0.5'>
                              Run this function automatically based on a cron schedule
                            </p>
                          </div>
                          <Switch
                          ...
                          <div className='mt-4 space-y-3 pl-0'>
                            <div className='space-y-1.5'>
                              ...
                              <p className='text-xs text-muted-foreground'>
                                ...
                                <br />
                                Examples: "*/5 * * * *" (every 5 min), "0 * * * *" (hourly)
                              </p>
                            </div>
                            ...
                            {scheduleSettings.last_scheduled_execution && (
                              <p className='text-sm text-muted-foreground'>
                                <strong>Last scheduled execution:</strong>{' '}
                                {formatDate(scheduleSettings.last_scheduled_execution)}
                              </p>
                            )}
                          </div>
                        ) : (
                    ...
                    <div className='flex items-center justify-between flex-wrap gap-3'>
                      <h2 className='text-base font-semibold flex items-center gap-2 text-foreground'>
                        ...
                        <Badge variant='secondary'>
                          {logsFilter === 'all'
                            ? `${logsPagination.totalCount} total`
                            : `${logsPagination.totalCount} ${logsFilter}`}
                        </Badge>
                      </h2>
                      <div className='flex items-center gap-2 flex-wrap'>
                        ...
                        <Button
                          variant='outline'
                          size='sm'
                          onClick={() => fetchExecutionLogs(logsCurrentPage, logsPageSize, logsFilter)}
                      ...
                      <div className='py-12 text-center text-muted-foreground'>
                        ...
                        <p className='text-sm'>
                          {logsPagination.totalCount === 0
                            ...
                            : 'Try adjusting your filter or refresh'}
                        </p>
                      </div>
                    ) : (
                      <div className='overflow-x-auto'>
                            ...
                            {executionLogs.map(log => {
                              ...
                              return (
                                <TableRow key={log.id}>
                                  ...
                                  <TableCell>
                                    <Badge variant={getStatusBadgeVariant(status ?? 0)}>{status ?? '—'}</Badge>
                                  </TableCell>
                                  <TableCell className='text-sm'>{duration ?? '—'}ms</TableCell>
                                  <TableCell className='text-sm text-muted-foreground'>
                                    {formatBytes(payload?.request?.body?.size)}
                                  </TableCell>
                                  <TableCell className='text-sm text-muted-foreground'>
                                    {formatBytes(payload?.response?.body?.size)}
                                  </TableCell>
                                  ...
                                  <TableCell>
                                    <Button
                                      variant='ghost'
                                      size='sm'
                                      disabled={!log.payload?.trace_id}
                                      onClick={() =>
                                        log.payload?.trace_id &&
                                        router.push(`/admin/logs/trace/${log.payload.trace_id}`)
                                      }
                    ...
                    {logsPagination.totalPages > 1 && (
                      <div className='flex items-center justify-between pt-4 border-t border-border'>
                        <p className='text-sm text-muted-foreground'>
                          ...
                          {Math.min(logsPagination.currentPage * logsPagination.limit, logsPagination.totalCount)} of{' '}
                          {logsPagination.totalCount}
                        </p>
                        <div className='flex items-center gap-1'>
                          ...
                          {Array.from({ length: Math.min(5, logsPagination.totalPages) }, (_, i) => {
                            ...
                            return (
                              <Button
                                key={pageNum}
                                variant={pageNum === logsPagination.currentPage ? 'default' : 'ghost'}
                                size='sm'
                    ...
                    <div className='pt-2 py-8'>
                      <div className='flex items-center justify-between gap-4'>
                        <div className='flex-1'>
                          ...
                          <p className='text-sm text-muted-foreground mt-0.5'>
                            Require an API key to execute this function. <br />
                            <br />
                            For API gateway upstream, the API authentication will be bypassed. <br />
                            Recommended to configure the authentication in the API gateway settings instead.
                            <br />
                            <br />
                            Turn this on if you want to ensure the function is executed from the API gateway.
                          </p>
                        </div>
                        <Switch checked={requiresApiKeyDraft} onCheckedChange={setRequiresApiKeyDraft} />
                      </div>
                      {requiresApiKeyDraft && functionData.requires_api_key && (
                        <div className='mt-4 space-y-2 pl-0'>
                          ...
                          <div className='flex min-w-0'>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                ...
                                >
                                  {copiedUrl === functionData.api_key ? (
                              ...
                              <TooltipContent>
                                {copiedUrl === functionData.api_key ? 'Copied!' : 'Copy API key'}
                              </TooltipContent>
                            </Tooltip>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  ...
                                  className='rounded-l-none border-l-0 bg-yellow-600/10 hover:bg-yellow-600/20'
                                >
                          ...
                          <p className='text-xs text-muted-foreground'>
                            Include as: Authorization: Bearer &lt;key&gt; or ?api_key=&lt;key&gt;
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className='py-8'>
                      <div className='flex items-center justify-between gap-4'>
                        <div className='flex-1'>
                          ...
                          <p className='text-sm text-muted-foreground mt-0.5'>
                            Override the global default timeout for this function. Not needed for most functions — only
                            enable this for functions that are known to require longer execution times.
                            <br />
                            {customTimeoutEnabled ? 'Global default' : 'Current setting'}:{' '}
                            <span className='font-medium text-foreground'>{globalExecSettings.defaultTimeout}s</span>
                            {customTimeoutEnabled ? (
                              <>
                                {' '}
                                &nbsp;·&nbsp; Maximum:{' '}
                                <span className='font-medium text-foreground'>{globalExecSettings.maxTimeout}s</span>
                              </>
                            ) : null}
                          </p>
                        </div>
                      ...
                      <div className='flex items-center justify-between gap-4'>
                        <div className='flex-1'>
                          ...
                          <p className='text-sm text-muted-foreground mt-0.5'>
                            ...
                            Values must be multiples of 256 MB (e.g. 256, 512, 768, 1024). You can use a suffix:{' '}
                            <code className='text-xs'>512M</code> or <code className='text-xs'>1G</code>.
                            <br />
                            {customMemoryEnabled ? 'Global default' : 'Current setting'}:{' '}
                            <span className='font-medium text-foreground'>{globalExecSettings.defaultMemory} MB</span>
                            {customMemoryEnabled ? (
                              <>
                                {' '}
                                &nbsp;·&nbsp; Maximum:{' '}
                                <span className='font-medium text-foreground'>{globalExecSettings.maxMemory} MB</span>
                              </>
                            ) : null}
                          </p>
                        </div>
          ...
          >
            <Tabs defaultValue='upload' className='mt-2'>
              ...
              <TabsContent value='upload' className='space-y-4 mt-4'>
                <div
                  className={cn(
                    ...
                    deployFile ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'
                  )}
                ...
                >
                  {deployFile ? (
                    <div className='space-y-3'>
                      ...
                      <div>
                        <p className='font-medium text-foreground'>{deployFile.name}</p>
                        <p className='text-muted-foreground text-sm'>{formatBytes(deployFile.size)}</p>
                      </div>
                      <Button
                        variant='ghost'
                        size='sm'
                    ...
                    <div className='space-y-3'>
                      ...
                      <div>
                        ...
                        <p className='text-muted-foreground text-sm'>or click to browse (.zip, .tar.gz, .tgz)</p>
                      </div>
                      <input
                        ref={deployFileInputRef}
                        type='file'
                        accept='.zip,.tar.gz,.tgz'
                        onChange={handleDeployFileSelect}
                        className='hidden'
                      />
                      <Button
                        variant='outline'
                        ...
                        onClick={e => {
                          e.stopPropagation()
                          deployFileInputRef.current?.click()
                        }}
                      >
                        Choose File
                      </Button>
                ...
                {uploadResult && (
                  <div
                    className={cn(
                      ...
                      uploadResult.success ? 'border-green-700 bg-green-900/10' : 'border-red-700 bg-red-900/10'
                    )}
                  >
                    ...
                    <span className={cn('text-sm', uploadResult.success ? 'text-green-300' : 'text-red-300')}>
              ...
              <TabsContent value='cli' className='space-y-4 mt-4'>
                <p className='text-sm text-muted-foreground'>
                  Deploy your function directly from the command line using the Invoke CLI.
                </p>
                <div className='space-y-4'>
                  <div>
                    ...
                    <pre className='bg-muted rounded-md p-3 text-sm font-mono text-foreground overflow-x-auto'>
                ...
                <div className='bg-blue-900/20 border border-blue-700 rounded-lg p-3'>
                  <p className='text-blue-300 text-sm'>
                    Run <code className='text-blue-200 bg-blue-900/50 px-1 rounded'>invoke --help</code> to see all
                    available options.
                  </p>
                </div>
              </TabsContent>
          ...
          >
            <div className='space-y-4 py-2'>
              ...
              {retentionDraft.retention_enabled && (
                <div className='space-y-4 pl-6 border-l border-border'>
                  <div className='space-y-1.5'>
                    ...
                    <Select
                      value={retentionDraft.retention_type}
                      onValueChange={v => setRetentionDraft(prev => ({ ...prev, retention_type: v }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  {retentionDraft.retention_type !== 'none' && (
                    <div className='space-y-1.5'>
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setRetentionDraft(prev => ({ ...prev, retention_value: parseInt(e.target.value) || 1 }))
                        }
                        min='1'
                      />
                      <p className='text-xs text-muted-foreground'>
                        {retentionDraft.retention_type === 'time'
              ...
              <div className='flex gap-2 pt-2'>
                ...
                <Button variant='outline' onClick={() => setRetentionModalOpen(false)}>
                  <X className='w-4 h-4 mr-1' />
                  Cancel
                </Button>
              </div>
