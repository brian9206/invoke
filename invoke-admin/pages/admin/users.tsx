  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (passwordScore < 3) {
      setDialogState({
        type: 'alert',
        title: 'Weak Password',
        message: 'Password is not strong enough. Please use a stronger password with a score of at least 3.'
      })
      return
    }
    try {
  ...
  const handleDeleteUser = async (user: User) => {
    if (currentUser && user.id === currentUser.id) {
      setDialogState({
        type: 'alert',
        title: 'Cannot Delete',
        message: 'You cannot delete your own account. Please use another admin account to delete this user.'
      })
      return
    }
    setDialogState({
                    ...
                    <div className='flex items-center gap-2'>
                      <div
                        className={`p-1.5 rounded shrink-0 ${
                          user.is_admin ? 'bg-purple-900/30 text-purple-400' : 'bg-green-900/30 text-green-400'
                        }`}
                      >
                      ...
                      <div className='flex-1 min-w-0'>
                        <div className='flex items-center flex-wrap gap-1.5'>
                          ...
                          {user.is_admin && (
                            <Badge variant='purple' className='text-xs px-1.5 py-0'>
                              Admin
                            </Badge>
                          )}
                        </div>
                          ...
                          return (
                            <div className='flex items-center gap-1 mt-0.5'>
                              ...
                              {isRecent ? (
                                <Badge
                                  variant='default'
                                  className='text-xs px-1.5 py-0 bg-green-900/30 text-green-400 border-green-800/50'
                                >
                                  Recent
                                </Badge>
                              ) : (
          ...
          >
            <form onSubmit={handleCreateUser} data-create-user className='space-y-4'>
              ...
              <div className='flex items-center gap-2'>
                ...
                />
                <Label htmlFor='createIsAdmin' className='cursor-pointer'>
                  Admin User
                </Label>
              </div>
            </form>
            ...
            <form onSubmit={handleUpdateUser} data-edit-user className='space-y-4'>
              ...
              <div className='flex items-center gap-2'>
                ...
                <Label
                  htmlFor='editIsAdmin'
                  className={
                    currentUser && editingUser && currentUser.id === editingUser.id ? 'opacity-50' : 'cursor-pointer'
                  }
                >
                  Admin User
                </Label>
