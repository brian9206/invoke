...
export default function Logs() {
  ...
  const fetchLogs = useCallback(
    async (page: number, limit: number, q: string, projectId: string, from: Date, to: Date, type: string) => {
      setLoading(true)
      try {
        ...
        <Layout title='Monitoring'>
          <div className='flex justify-center items-center h-64'>
            <div className='flex items-center gap-2 text-muted-foreground'>
              ...
              <span className='animate-pulse'>{projectLoading ? 'Loading projects...' : 'No project selected'}</span>
            </div>
          </div>
        </Layout>
      </ProtectedRoute>
      ...
      <Layout title='Monitoring'>
        <div className='flex flex-col h-[calc(100vh-var(--header-height,4rem)-2rem)] gap-3'>
          ...
          <div className='flex items-center gap-2 flex-shrink-0 flex-wrap'>
            ...
            <KqlSearchBar onSearch={handleSearch} initialValue={kqlQuery} />
            <div className='flex items-center gap-2 flex-shrink-0'>
              ...
              <Button
                ...
                onClick={() => {
                  const from = timeRange.fromExpr
                    ? (parseExprToDate(timeRange.fromExpr) ?? timeRange.from)
                    : timeRange.from
                  const to = timeRange.toExpr ? (parseExprToDate(timeRange.toExpr) ?? timeRange.to) : timeRange.to
                  fetchLogs(currentPage, pageSize, kqlQuery, activeProject.id, from, to, logType)
                }}
