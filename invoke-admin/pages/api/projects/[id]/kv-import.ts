...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!hasAccess.canWrite) {
      return res
        .status(403)
        .json(createResponse(false, null, 'Developer or owner role required for import operations', 403))
    }
    ...
    const limitSetting = await GlobalSetting.findOne({
      ...
      return res
        ...
        .json(
          createResponse(
            false,
            null,
            `Import would exceed storage quota. Projected usage: ${projectedMB}MB, Limit: ${limitMB}MB`,
            413
          )
        )
    }
