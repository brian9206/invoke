...
import { ChevronDown, ChevronRight, GripVertical, Plus, Pencil, Trash2, Check, X, FolderOpen } from 'lucide-react'
...
function GroupTreeNode({
  ...
  const commitEdit = async () => {
    ...
    if (trimmed.includes('/')) {
      setEditError('Group name cannot contain /')
      return
    }
    if (trimmed === node.displayName) {
      cancelEdit()
      return
    }
  ...
  const saveSubGroup = async () => {
    ...
    try {
      const res = await authenticatedFetch(`/api/function-groups?projectId=${encodeURIComponent(projectId)}`, {
        method: 'POST',
    ...
    <div ref={setSortRef} style={style}>
      ...
      <div
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-lg border group/header transition-colors ${
          isDropOver && canWrite ? 'bg-primary/10 border-primary/40' : 'bg-muted/40 border-border/50'
        }`}
      ...
      >
        ...
        {isProjectFake && <FolderOpen className='w-4 h-4 text-muted-foreground shrink-0' />}
        ...
        >
          ...
          {editing ? (
            <span className='flex flex-col flex-1 gap-1' onClick={e => e.stopPropagation()}>
              ...
              {editError && <span className='text-xs text-red-400 pl-1'>{editError}</span>}
            </span>
          ) : (
            <span className='font-medium text-sm text-foreground truncate'>{node.displayName}</span>
          )}

          {!editing && <span className='ml-1 text-xs text-muted-foreground shrink-0'>({functionsInGroup.length})</span>}
        </button>

        {canWrite && !editing && !isProjectFake && (
          <div className='flex items-center gap-1'>
            {node.depth < MAX_DEPTH && (
              <Button variant='ghost' size='icon' className='h-7 w-7' title='Add sub-group' onClick={openSubGroupAdd}>
      ...
      {isOpen && (
        <div className='mt-1 space-y-1'>
          ...
          {!isProjectFake && (functionsInGroup.length > 0 || node.children.length === 0) && (
            <SortableContext items={functionsInGroup.map(f => f.id)} strategy={verticalListSortingStrategy}>
              <div className='space-y-2 min-h-[2rem] rounded-lg transition-colors' style={{ marginLeft: indent + 24 }}>
    ...
    <div ref={setNodeRef}>
      <button
        ...
        className={`flex items-center gap-2 px-3 py-2 w-full text-left rounded-lg border border-dashed transition-colors ${
          isOver && canWrite ? 'bg-primary/10 border-primary/40' : 'bg-muted/20 border-border/50 hover:bg-muted/30'
        }`}
      >
        {isOpen ? (
      ...
      {isOpen && (
        ...
        >
          {functions.length === 0 ? (
            <p className='text-xs text-muted-foreground py-2 text-center'>No ungrouped functions</p>
          ) : (
...
function AddGroupRow({ projectId, onCreated }: { projectId: string; onCreated: (group: FunctionGroup) => void }) {
  ...
  if (!adding) {
    return (
      <Button variant='ghost' size='sm' onClick={open} className='text-muted-foreground w-full justify-start gap-2'>
...
export function FunctionGroupList({
  ...
  const ungrouped = functions.filter(f => !f.group_id).sort((a, b) => a.sort_order - b.sort_order)

  const toggleGroupOpen = (id: string) => setOpenGroups(prev => ({ ...prev, [id]: prev[id] !== false ? false : true }))
  ...
  const siblingsOf = useCallback(
    (fullPath: string): FunctionGroup[] => {
      ...
      return groups.filter(g => parentPathOf(g.name) === parent).sort((a, b) => a.sort_order - b.sort_order)
    },
    [groups]
  )
  ...
  const saveReorder = useCallback(
    async (
      ...
      try {
        await authenticatedFetch(`/api/function-groups/reorder?projectId=${encodeURIComponent(projectId)}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeId.startsWith('group:') && overId.startsWith('nest:')) {
      ...
      if (nestNode.fullPath === activeNode.fullPath || nestNode.fullPath.startsWith(activeNode.fullPath + '/')) return
      ...
      try {
        ...
        if (data.success) {
          ...
          const deletedIds = new Set(
            groups.filter(g => g.name === nodePath || g.name.startsWith(nodePath + '/')).map(g => g.id)
          )
          onGroupsChange(groups.filter(g => !deletedIds.has(g.id)))
          onFunctionsChange(
            functions.map(f => (f.group_id && deletedIds.has(f.group_id) ? { ...f, group_id: null } : f))
          )
        }
      } catch (e) {
    ...
    if (!activeId.startsWith('group:')) {
      ...
      let newFunctions = functions.map(f => (f.id === funcId ? { ...f, group_id: destGroupId } : f))

      const destList = newFunctions.filter(f => f.group_id === destGroupId).sort((a, b) => a.sort_order - b.sort_order)
      ...
      if (!overId.startsWith('group:') && !overId.startsWith('nest:') && overId !== 'ungrouped') {
        ...
        newFunctions = newFunctions.map(f => (f.id === funcId ? { ...f, sort_order: maxSortOrder + 1 } : f))
      }
  ...
  const handleDeleteGroup = async (node: TreeNode) => {
    try {
      ...
      if (data.success) {
        ...
        onGroupsChange(groups.filter(g => !deletedIds.has(g.id)))
        onFunctionsChange(functions.map(f => (f.group_id && deletedIds.has(f.group_id) ? { ...f, group_id: null } : f)))
      }
    } catch (e) {
  ...
  const activeDragFunc =
    activeDragId && !activeDragId.startsWith('group:') ? functions.find(f => f.id === activeDragId) : null
    ...
    <TooltipProvider>
      ...
      >
        <div className='space-y-3'>
          <SortableContext items={tree.map(n => `group:${n.group.id}`)} strategy={verticalListSortingStrategy}>
            {tree.map(node => (
              <GroupTreeNode
                key={node.group.id}
                node={node}
          ...
          {canWrite && <AddGroupRow projectId={projectId} onCreated={handleGroupCreated} />}
        </div>
