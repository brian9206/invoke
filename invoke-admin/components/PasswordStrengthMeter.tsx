...
const PasswordStrengthMeter: React.FC<PasswordStrengthMeterProps> = ({
  ...
  const feedback =
    ...
    (result.feedback.suggestions.length > 0 ? result.feedback.suggestions[0] : '');

  return (
    <div className="mt-2 space-y-2">
      <Progress
        value={(score / 4) * 100}
        className={`h-1.5 bg-secondary ${progressColors[score]}`}
      />
      <div className="text-xs space-y-1">
        <span className={`font-medium ${textColors[score]}`}>
