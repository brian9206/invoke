...
export function makeDynamicColumnDef(fieldPath: string): ColumnDef {
  ...
  return {
    ...
    render: log => {
      ...
      return (
        <span className='text-xs font-mono text-muted-foreground truncate max-w-[180px] block' title={display}>
          {display}
        </span>
      )
    }
                      ...
                      <table className='w-full text-xs'>
                        <tbody>
                          {flattenPayload(log.payload).map(([field, value]) => (
                            <tr key={field} className='group border-b border-border/40 last:border-0 hover:bg-muted/30'>
...
export const ALL_COLUMN_DEFS: ColumnDef[] = [
  ...
  },
  {
    ...
    render: log => {
      ...
      return url ? (
        <span className='text-xs font-mono text-muted-foreground truncate max-w-[240px] block' title={url}>
          {url}
        </span>
      ) : (
        <span className='text-muted-foreground text-xs'>—</span>
  ...
  },
  {
    ...
    render: log => {
      ...
      return v ? (
        <span className='text-xs font-mono text-muted-foreground truncate max-w-[160px] block' title={String(v)}>
          {String(v)}
        </span>
      ) : (
        <span className='text-muted-foreground text-xs'>—</span>
  ...
  },
  {
    ...
    render: log => {
      ...
      return v ? (
        <span className='text-xs truncate max-w-[280px] block' title={String(v)}>
          {String(v)}
        </span>
      ) : (
        <span className='text-muted-foreground text-xs'>—</span>
  ...
  },
  {
    ...
    render: log => {
      ...
      return error ? (
        <span className='text-red-400 text-xs max-w-[200px] truncate block' title={error}>
  ...
  },
  {
    ...
    render: log => {
      ...
      return v ? (
        <span className='text-xs truncate max-w-[280px] block' title={'v' + String(v)}>
          v{String(v)}
        </span>
      ) : (
        <span className='text-muted-foreground text-xs'>—</span>
...
export function getDefaultColumnKeys(type: 'request' | 'app' | 'build'): string[] {
  switch (type) {
    case 'request':
      return DEFAULT_HTTP_COLUMN_KEYS
    case 'app':
      return DEFAULT_APP_COLUMN_KEYS
    case 'build':
      return DEFAULT_BUILD_COLUMN_KEYS
    default:
