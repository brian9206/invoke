...
function MiniCalendar({ value, onChange }: { value: Date; onChange: (d: Date) => void }) {
  ...
  for (let i = 0; i < firstDow; i++) cells.push({ day: prevMonDays - firstDow + 1 + i, t: 'prev' })
  for (let d = 1; d <= daysInMonth; d++) cells.push({ day: d, t: 'cur' })
  let nxt = 1
...
function TimeSlotList({ value, onChange }: { value: string; onChange: (t: string) => void }) {
  ...
  const slots: string[] = []
  for (let h = 0; h < 24; h++) for (const m of [0, 30]) slots.push(`${pad2(h)}:${pad2(m)}`)
    ...
    <div className='flex flex-col flex-1 min-w-0 gap-3'>
      <Tabs value={state.mode} onValueChange={v => onChange({ ...state, mode: v as EndpointMode })}>
        ...
        <div className='flex items-start justify-between gap-2 flex-wrap mb-2'>
          <TabsList className='h-7'>
            <TabsTrigger value='absolute' className='text-xs px-2.5 h-6'>
              Absolute
            </TabsTrigger>
            <TabsTrigger value='relative' className='text-xs px-2.5 h-6'>
              Relative
            </TabsTrigger>
            <TabsTrigger value='now' className='text-xs px-2.5 h-6'>
              Now
            </TabsTrigger>
          </TabsList>
        ...
        <TabsContent value='now' className='mt-0'>
          <p className='text-xs text-muted-foreground leading-relaxed py-3'>
            Setting the time to &ldquo;now&rdquo; means that on every refresh this time will be set to the time of the
            refresh.
          </p>
      ...
      {state.mode === 'absolute' && (
        <p className='text-[11px] text-muted-foreground border-t border-border pt-2 mt-auto'>
          <span className='font-medium text-foreground capitalize'>{title} date</span> {resolvedStr}
        </p>
      )}
    </div>
  )
...
export function TimeRangePicker({ value, onChange }: TimeRangePickerProps) {
  ...
  const handleStartOpenChange = (open: boolean) => {
    if (open) {
      setStartState(initFrom(value.from))
      setStartError('')
    }
    setStartOpen(open)
  }
  ...
  const handleUpdateStart = () => {
    ...
    if (from >= value.to) {
      setStartError('"Start" must be before the current end.')
      return
    }
    ...
    onChange({
      from,
      to: value.to,
      label: `${endpointLabel(startState)} → ${right}`,
      fromExpr: endpointExpr(startState),
      toExpr: value.toExpr
    })
    setStartOpen(false)
  }
  ...
  const handleUpdateEnd = () => {
    ...
    if (value.from >= to) {
      setEndError('"End" must be after the current start.')
      return
    }
    ...
    onChange({
      from: value.from,
      to,
      label: `${left} → ${endpointLabel(endState)}`,
      fromExpr: value.fromExpr,
      toExpr: endpointExpr(endState)
    })
    setEndOpen(false)
  }
    ...
    <div className='flex items-center h-9 border border-border rounded-md text-xs font-mono bg-background shrink-0 pe-2'>
      ...
      <Popover open={startOpen} onOpenChange={handleStartOpenChange}>
        ...
        >
          ...
          <div className='flex justify-end gap-2 pt-3 border-t border-border mt-3'>
            <Button variant='outline' size='sm' onClick={() => setStartOpen(false)}>
              Cancel
            </Button>
            <Button size='sm' onClick={handleUpdateStart}>
              Update
            </Button>
          </div>
        </PopoverContent>
      ...
      <Popover open={endOpen} onOpenChange={handleEndOpenChange}>
        ...
        >
          ...
          <div className='flex justify-end gap-2 pt-3 border-t border-border mt-3'>
            <Button variant='outline' size='sm' onClick={() => setEndOpen(false)}>
              Cancel
            </Button>
            <Button size='sm' onClick={handleUpdateEnd}>
              Update
            </Button>
          </div>
        </PopoverContent>
