...
export async function dbInsertLog(opts: DbInsertLogOptions): Promise<void> {
  ...
  if (source !== 'execution' && source !== 'gateway') throw new Error(`[dbInsertLog] Invalid source: ${source}`)
  ...
  if (functionArg?.id && (!resolvedProjectId || resolvedFunctionName === null || resolvedProjectName === null)) {
    try {
      ...
      if (func) {
        ...
        if (resolvedProjectName === null) resolvedProjectName = (func as any).Project?.get('name') ?? null
      }
    } catch (lookupErr) {
