...
const SHARED_EXCEPTIONS = new Set(['PORT'])
...
async function main() {
  ...
  const sharedKeysOrdered = [...sharedKeys].sort((a, b) => keyMeta.get(a).firstOrder - keyMeta.get(b).firstOrder)

  // 3. Overwrite check
  const existing = available.map(svc => path.join(ROOT, svc.dir, '.env')).filter(p => fs.existsSync(p))
  ...
  for (const svc of available) {
    const uniqueEntries = parsed[svc.name].filter(e => e.type === 'var' && !sharedKeys.has(e.key))
