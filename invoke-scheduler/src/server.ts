...
async function triggerScheduledFunctions(): Promise<void> {
  ...
  try {
    ...
    console.error(`[${new Date().toISOString()}] Error triggering scheduled functions:`, message)
  } finally {
...
async function triggerCleanupLogs(): Promise<void> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error(`[${new Date().toISOString()}] Log cleanup request failed:`, response.status, response.statusText)
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
