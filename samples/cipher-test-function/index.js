...
module.exports = async function (req, res) {
  ...
  try {
    ...
    const gcmEncrypted = Buffer.concat([gcmCipher.update(gcmPlaintext, 'utf8'), gcmCipher.final()])
    ...
    const gcmDecrypted = Buffer.concat([gcmDecipher.update(gcmEncrypted), gcmDecipher.final()]).toString('utf8')
    ...
    const ctrEncrypted = Buffer.concat([ctrCipher.update(ctrPlaintext, 'utf8'), ctrCipher.final()])

    const ctrDecipher = crypto.createDecipheriv('aes-256-ctr', ctrKey, ctrIv)
    const ctrDecrypted = Buffer.concat([ctrDecipher.update(ctrEncrypted), ctrDecipher.final()]).toString('utf8')
    ...
    const chunkDecrypted = Buffer.concat([chunkDecipher.update(chunkEncrypted), chunkDecipher.final()]).toString('utf8')
    ...
    const withPadding = Buffer.concat([padCipher.update('Short', 'utf8'), padCipher.final()])
    ...
    const noPadding = Buffer.concat([noPadCipher.update(exactBlock), noPadCipher.final()])
