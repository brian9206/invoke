...
module.exports = async function (req, res) {
  ...
  try {
      ...
      await new Promise((resolve, reject) => {
        ...
        socket.on('close', () => {
          ...
          results.connectivity.httpbinValidHTTP =
            responseData.includes('HTTP/1.1 200') && responseData.includes('application/json')
          resolve()
        })
    ...
    results.summary = {
      ...
      moduleFullyLoaded:
        results.moduleLoading.loaded && results.moduleLoading.hasConnect && results.moduleLoading.hasTLSSocket,
      certificatesWorking: results.certificateStore.getCACertificatesWorks,
      serverStubsWorking: results.serverStubs.createServerThrows && results.serverStubs.serverConstructorThrows,
      compatibilityGood: results.compatibility.connectOverloadsWork && results.compatibility.checkServerIdentityValid,
      connectivityWorking:
        results.connectivity && (results.connectivity.httpbinConnect || results.connectivity.exampleConnect),
      realTLSFunctional:
        results.connectivity && results.connectivity.httpbinResponseReceived && results.connectivity.httpbinValidHTTP
    }
