...
async function build(typesDir, options) {
  ...
  for (const file of files) {
    ...
    for (const line of lines) {
      ...
      if (/^export\s+\{\s*type\s+\w+[^}]*\}\s*;?\s*$/.test(trimmed)) continue

      // Remove bare export default lines (e.g. `export default _default;`)
      if (
        /^export\s+default\s+[^({\s]+\s*;?\s*$/.test(trimmed) &&
        !/^export\s+default\s+(function|class)\s/.test(trimmed)
      )
        continue

      // Remove `declare const _default: import(...) ...` lines (generated by tsc for default exports)
      if (/^declare\s+const\s+_default\s*:/.test(trimmed)) continue
      ...
      let processed = line
      processed = processed.replace(/^(\s*)export\s+default\s+(function|class)\s/, '$1$2 ')
      ...
      processed = processed.replace(/^(\s*)declare\s+(const|var|function|class|enum)\s/, '$1$2 ')
