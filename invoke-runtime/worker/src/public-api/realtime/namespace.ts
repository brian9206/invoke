...
for (const method of ['join', 'leave']) {
  (RealtimeNamespaceFactory.prototype as any)[method] = function () {
    throw new Error(
      `${method}() is not available on namespace — use ns.socket.${method}() inside an event handler`,
    );
  };
}
...
Object.defineProperty(RealtimeNamespaceFactory.prototype, 'broadcast', {
  get() {
    throw new Error(
      'broadcast is not available on namespace — use ns.socket.broadcast inside an event handler',
    );
  },
  configurable: true,
});
...
RealtimeNamespaceFactory.prototype._dispatch = async function (
  req: any,
  res: any,
  next?: any,
): Promise<void> {
  ...
  const isSocketRequest =
    (req && req.path) === '/socket.io' && !!headers['x-realtime-socket-event'];
  ...
  const disconnectReasonHeader: string | null =
    headers['x-realtime-socket-disconnect-reason'] || null;
  ...
  const disconnectReason: string | null =
    ...
    (event === '$disconnect' ? ((handshake as any).disconnectReason ?? null) : null);

  self.socket._hydrate(
    socketId,
    rooms,
    handshake as any,
    namespace || null,
