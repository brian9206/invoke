...
import type { Stage, BuildStatus, BuildContext, Pipeline } from './types'
...
export class PipelineRunner extends EventEmitter {
  ...
  async run(buildData: BuildData): Promise<void> {
    const stageMap = new Map<string, Stage>(this.pipeline.stages.map(s => [s.name, s]))
    ...
    const inDegree = new Map<string, number>(this.pipeline.stages.map(s => [s.name, 0]))
    const dependents = new Map<string, string[]>(this.pipeline.stages.map(s => [s.name, []]))
    ...
    let ready = this.pipeline.stages.filter(s => inDegree.get(s.name) === 0).map(s => s.name)
    let processed = 0
        ...
        batch.map(async stageName => {
          ...
          try {
            ...
            const error = new Error(
              `Stage "${stageName}" failed: ${stageStatus[stageName].error}`
            ) as PipelineFailureError
