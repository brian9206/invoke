...
function extractBasicCredentials(authorizationHeader: string | undefined): BasicCredentials | null {
  if (!authorizationHeader) return null
...
function validateBasicAuth(req: Request, config: Record<string, any>): AuthResult {
  ...
  const matchFound = credentials.some(c => c.username === creds.username && c.password === creds.password)
